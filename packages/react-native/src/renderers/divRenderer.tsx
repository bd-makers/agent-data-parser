...
const hasImageInElement = (elem: React.ReactElement, context: IRendererContext): boolean => {
  ...
  if (elem.type === BDView) {
    const props = elem.props as { children?: ReactNode; style?: Record<string, unknown> };
...
const applyTextElementStyles = (
  ...
  elements.map((elem, idx) => {
    ...
    const props = elem.props as { style?: Record<string, unknown> };
      ...
      {finalTextElements.length > 0 && (
        ...
        >
          {finalTextElements.map((elem, idx) => {
            if (React.isValidElement(elem)) {
              const existingStyle =
                (elem.props as { style?: Record<string, unknown> })?.style || {};
              return React.cloneElement(elem, {
